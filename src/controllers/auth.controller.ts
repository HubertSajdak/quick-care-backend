...
import {default as DoctorModel, default as UserModel, DoctorDocument} from "../models/doctor.model";
...
import DoctorSpecializationModel from "../models/doctorSpecialization.model";
...
export const getUserData = async (req: AuthenticatedUserValues, res: Response) => {
    ...
    if (isPatient) {
        const {_id, address, name, surname, phoneNumber, email, photo, role, birthDate, height, weight} = isPatient;
...
export const updateUserInfo = async (req: AuthenticatedUserValues, res: Response) => {
    ...
    if (isDoctor) {
        ...
        if (isDoctor && isDoctor.email.toString() === process.env.DOCTOR_DEMO_EMAIL!.toString() && (email !== process.env.DOCTOR_DEMO_EMAIL!.toString() || name !== process.env.DOCTOR_DEMO_NAME!.toString() || surname !== process.env.DOCTOR_DEMO_SURNAME!.toString())) {
    ...
    if (isPatient) {
        const {name, surname, email, phoneNumber, address, birthDate, height, weight} = req.body;
        if (!name || !surname || !email || !phoneNumber || !address || !birthDate || !height || !weight) {
            throw new BadRequestError("errors.BAD_OBJECT_STRUCTURE");
        }
        if (isPatient && isPatient.email.toString() === process.env.PATIENT_DEMO_EMAIL!.toString() && (email !== process.env.PATIENT_DEMO_EMAIL!.toString() || name !== process.env.PATIENT_DEMO_NAME!.toString() || surname !== process.env.PATIENT_DEMO_SURNAME!.toString())) {
            throw new BadRequestError("errors.DEMO_ACCOUNT_ERROR")
        }
...
export const updateUserPassword = async (req: AuthenticatedUserValues, res: Response) => {
    ...
    if (isPatient && isPatient.email.toString() === process.env.PATIENT_DEMO_EMAIL!.toString() || isDoctor && isDoctor.email.toString() === process.env.DOCTOR_DEMO_EMAIL!.toString()) {
        throw new BadRequestError("errors.DEMO_ACCOUNT_ERROR")
    }
...
export const deleteAccount = async (req: AuthenticatedUserValues, res: Response) => {
    const user = req.user;
    if (user?.userId === process.env.DOCTOR_DEMO_ID!.toString() || user?.userId === process.env.PATIENT_DEMO_ID!.toString()) {
        throw new BadRequestError("errors.DEMO_ACCOUNT_ERROR")
    }
    if (!user) {
        throw new UnauthenticatedError("errors.INVALID_AUTHENTICATION");
    ...
    if (isDoctor) {
        await AppointmentModel.deleteMany({doctorId: user.userId})
        await ClinicAffiliationModel.deleteMany({doctorId: user.userId})
        await DoctorSpecializationModel.deleteMany({doctorId: user.userId})
        await DoctorModel.deleteOne({_id: user.userId})
    }
    if (isPatient) {
        await AppointmentModel.deleteMany({patientId: user.userId})
        await PatientModel.deleteOne({_id: user.userId})
    }
    res.status(StatusCodes.OK).json({message: `${t("success.ACCOUNT_DELETED")}`});
